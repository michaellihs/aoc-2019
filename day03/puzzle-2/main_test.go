...
func TestIntersection_stepCount(t *testing.T) {
	wire1 := []Direction{NewDirection("R8"), NewDirection("U5"), NewDirection("L5"), NewDirection("D3")}
	wire2 := []Direction{NewDirection("U7"), NewDirection("R6"), NewDirection("D4"), NewDirection("L4")}
	expIntersect := Point{6,5}

	intersect := Intersection{expIntersect, 0, 0}
	count1, count2 := intersect.StepCount(wire1, wire2)

	assert.Equal(t, 15, count1)
	assert.Equal(t, 15, count2)
}
...
func TestFewestSteps(t *testing.T) {
	...
	fewestSteps := FewestSteps(lines, wire1, wire2)

	assert.Equal(t, 30, fewestSteps)
}

func TestFewestSteps_input_1(t *testing.T) {
	wire1, wire2 := ParseWires("/Users/mimi/workspace/learning/adventofcode/day03/puzzle-1/test-input-1.txt")
	lines := Path2Lines(wire1)

	fewestSteps := FewestSteps(lines, wire1, wire2)

	assert.Equal(t, 610, fewestSteps)
}